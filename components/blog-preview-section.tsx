    ...
    <section className="py-24">
      <Container>
        <div className="flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
          ...
          />
          <Link href="/blog" className="text-sm font-medium text-blue-600 transition hover:text-blue-500">
            Visit the blog
          </Link>
        </div>
        <div className="mt-12 grid gap-6 lg:grid-cols-3">
          {posts.map((post) => (
            <article
              key={post.slug}
              className="rounded-[2rem] border border-slate-200/80 bg-white/90 p-6 shadow-[0_20px_60px_rgba(15,23,42,0.05)]"
            >
